...
const MainContent = () => {
  ...
  const friends = expenses.filter((item) => 
   
     item.friendName !== undefined;
};
  ...
  const submitHandler = (e) => {
    ...
    if (type === "Loaned to Friend" || type === "Loaned from Friend") {
        ...
        <Card className="w-[40%]">
          ...
          <div className="flex flex-col">
            {expenses
              .filter((item) => {
                // console.log(item);
                return item.friendName !== undefined;
              })
              .map((item, index) => (
    ...
    <div className="flex  w-full items-start gap-10 text-start">
      ...
      <p
        className={`w-[5rem] text-center ${
          expense.type === "Spent" && "text-red-500"
        } ${
          (expense.type === "Earned" ||
