...
const MainContent = () => {
  ...
  const submitHandler = (e) => {
    ...
    const data = {
      ...
      subCategory,
      type,
      mul: type === "Spent" || type === "Loaned to Friend" ? -1 : 1,
    };
...
const ExpenseItem = ({ expense }) => {
  console.log(expense.mul + " " + expense.amount);
  return (
    <div className="flex  w-full items-start gap-10 text-start">
      ...
      <p
        className={`${
          expense.type === "Spent" || expense.type === "Loaned from Friend"
            ? "text-red-500"
            : "text-green-500"
        }`}
      >
        ${expense.mul * +expense.amount}
      </p>
    </div>
  );
};
